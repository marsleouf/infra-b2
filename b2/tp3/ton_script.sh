...
declare  target_dirname=$(awk -F'/' '{ print $NF }' <<< "${target_path%/}")

# Craft the backup full path and name
declare  backup_destination_dir="/opt/backup/"
declare  backup_date="$(date +%y%m%d_%H%M%S)"
declare  backup_filename="${target_dirname}_${backup_date}.tar.gz"
declare  backup_destination_path="${backup_destination_dir}/${backup_filename}"

# Informations about the User that must run this script
declare  backup_user_name="claquettechaussette"
declare i backup_user_uid=1001
declare i backup_user_umask=077

# The quantity of backup to keep for each directory
declare -i backups_quantity=7
declare i backups_quantity=$((backups_quantity+1))
